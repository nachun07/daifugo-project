...
export function setupGameHandlers(io: Server){
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('create_room', (data:{name:string})=>{
      ...
      const room:Room = { code, players: [{ id: socket.id, name: data.name || 'Player' }], state: undefined }
      rooms.set(code, room)
      socket.join(code)
    ...
    socket.on('action', (data:{code?:string, type:'play'|'pass', cards?: Card[]})=>{
      // find room
      ...
      const room = rooms.get(roomCode as string)
